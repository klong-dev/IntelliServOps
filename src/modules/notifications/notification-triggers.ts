...
export class NotificationTriggers {
  ...
  async onTicketReplied(payload: {
    ...
    await this.notifications.createAndPush({
      ...
      message: `${payload.staffName} đã trả lời yêu cầu hỗ trợ ${payload.ticketCode}.`,
  ...
  async onViewingRequestStaffAssigned(payload: {
    ...
    await this.notifications.createAndPush({
      ...
      message: `${payload.requesterName} vừa tạo lịch xem căn hộ.`,
  ...
  async onViewingRequestUserCancelled(payload: {
    ...
    await this.notifications.createAndPush({
      ...
      notificationType: NotificationType.warning,
      channel: NotificationChannel.push,
      title: 'Lịch xem đã bị hủy',
      message: 'Người dùng đã hủy lịch hẹn xem căn hộ.',
  ...
  async onViewingRequestConfirmedByStaff(payload: {
    ...
    await this.notifications.createAndPush({
      ...
      notificationType: NotificationType.success,
      channel: NotificationChannel.push,
      title: 'Lịch xem đã được xác nhận',
      message: 'Nhân viên đã xác nhận lịch xem căn hộ của bạn.',
      actionUrl: `/viewing-requests/appointments/${payload.appointmentId}`,
      actionLabel: 'Xem lịch hẹn',
      priority: Priority.high,
      relatedEntityType: 'Appointment',
  ...
  async onViewingRequestDeniedByStaff(payload: {
    ...
    await this.notifications.createAndPush({
      ...
      notificationType: NotificationType.warning,
      channel: NotificationChannel.push,
      title: 'Lịch xem đã bị từ chối',
      message: payload.reason?.trim()
        ? `Nhân viên đã từ chối lịch xem: ${payload.reason.trim()}`
        : 'Nhân viên đã từ chối lịch xem căn hộ của bạn.',
      actionUrl: `/viewing-requests/appointments/${payload.appointmentId}`,
      actionLabel: 'Xem lịch hẹn',
      priority: Priority.high,
      relatedEntityType: 'Appointment',
