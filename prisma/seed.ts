...
async function main() {
  ...
  const partner1 = await prisma.partner.upsert({
    ...
    create: {
      ...
      bankAccountName: 'VO VAN CHU NHA',
      commissionRate: 8.0,
      isVerified: true,
      isActive: true,
    },
  ...
  const partner2 = await prisma.partner.upsert({
    ...
    create: {
      ...
      bankAccountName: 'TRUONG THI DAU TU',
      commissionRate: 10.0,
      isVerified: true,
      isActive: true,
    },
  ...
  await prisma.room.createMany({
    data: [
      { apartmentId: apt1.id, roomName: 'Phòng ngủ Master', roomType: 'bedroom', area: 25.0, status: 'available' },
      { apartmentId: apt1.id, roomName: 'Phòng ngủ 2', roomType: 'bedroom', area: 15.0, status: 'available' },
      { apartmentId: apt1.id, roomName: 'Phòng khách', roomType: 'living_room', area: 30.0, status: 'available' },
      { apartmentId: apt1.id, roomName: 'Bếp', roomType: 'kitchen', area: 10.0, status: 'available' },
      { apartmentId: apt2.id, roomName: 'Phòng ngủ chính', roomType: 'bedroom', area: 20.0, status: 'occupied' },
      { apartmentId: apt2.id, roomName: 'Phòng khách', roomType: 'living_room', area: 25.0, status: 'occupied' },
    ],
  });
  ...
  const iot1 = await prisma.ioTDevice.create({
    data: {
      apartmentId: apt2.id,
  ...
  await prisma.appointment.create({
    data: {
      ...
      appointmentType: 'physical_viewing',
      status: 'scheduled',
